const averagePair = require ("./averagePair.js");

test(`expected [1, 2, 3], 2.5 to output true`, () => {
  expect(averagePair( [1, 2, 3], 2.5)).toBe(true);
});

test(`expected [1, 3, 3, 5, 6, 7, 10, 12, 19], 8 to output true`, () => {
  expect(averagePair([1, 3, 3, 5, 6, 7, 10, 12, 19], 8)).toBe(true);
});

test(`expected [-1, 0, 3, 4, 5, 6], 4.1 to output false`, () => {
  expect(averagePair([-1, 0, 3, 4, 5, 6], 4.1)).toBe(false);
});

test(`expected [], 4 to output false`, () => {
  expect(averagePair([], 4)).toBe(false);
});